...
if __name__=="__main__":
    ...
    rewards_0_ = config['ppo_agent']['rewards_0']
    rewards_1_ = config['ppo_agent']['rewards_1']
    ...
    ppo_agent = PPO(input_dim, hidden_dim, n_layers, dropout, 
                    ...
                    K_epochs, eps_clip, True, ppo_model, device)
            ...
            for batch_data in batch_progress:
                ...
                for i in range(batch_index.shape[0]):
                    ...
                    if 'mistralai' in llm_model:
                        ...
                        messages_list.append(((i,-1), messages_mistra))
                    else:
                        messages_list.append(((i,-1), messages))
                    ...
                    for j, k in enumerate(demon_index):
                        ...
                        if 'mistralai' in llm_model:
                            messages_mistra = [{"role":"user", "content":'\n'.join([x['content'] for x in messages[:2]])}] + messages[2:]
                            messages_list.append(((i,j), messages_mistra))
                        else:
                            messages_list.append(((i,j), messages))

                batch_response = llm_agent.generate(messages_list)
                batch_response = dict(batch_response)
                ...
                for i in range(batch_index.shape[0]):
                    reward_row = []
                    for j in range(-1,batch_index.shape[1]):
                        reward_cell = batch_response.get((i,j))
                        while reward_cell is None:
                            j = j - 1
                            reward_cell = batch_response.get((i,j))
                        # check the judge
                        model_judge = int(bool(re.findall(r'<Yes>', reward_cell)))
                        expert_judge = batch_data['expert_judge'][i]
                        reward_value = model_judge == expert_judge
                        reward_row.append(reward_value)
                    reward_mtrx.append(reward_row)
                reward_mtrx = torch.FloatTensor(reward_mtrx).to(device)

                batch_length = (batch_index >= 0).sum(dim=1)
                batch_correct = reward_mtrx[:,batch_length]
                baseline_correct = reward_mtrx[:,0]
                
                print('Baseline(zeroshot) Accuracy: {:.4f} - RL Accuracy: {:.4f} - Batch Demon Length: {:.2f}'.format(
                    baseline_correct.mean().item(), batch_correct.mean().item(), batch_length.float().mean().item())
                )

                # part 0 correctness reward 
                rewards_0 = reward_mtrx[:,1:] * 2 - 1

                # part 1 stop bonus reward
                rewards_1 = rewards_0 * (batch_index == -1)
                
                # we amplify the last step result, the multiplier depends on maxlen
                # rewards_0[:,-1] = rewards_0[:,-1] * max((rewards_0.size(1) - 1) * 2, 1)
                # rewards_0[:,-1] = rewards_0[:,-1] * 10
                
                ppo_agent.memory.rewards = rewards_0 * rewards_0_ + rewards_1 * rewards_1_
